...
## PR workflow
After completing all changes, always create a PR. For code changes, functionally test the feature first. Then run code review (CR) in a loop — fix all reported issues until CR returns no problems. Do not auto-merge — wait for explicit user approval before merging.
